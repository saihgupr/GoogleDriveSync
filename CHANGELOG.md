...
### Fixed
- Sync operations now use `--ignore-errors` and `--retries 3` flags, preventing a handful of locked or permission-denied files from aborting the entire sync session. This addresses errors like 'failed to delete N files' caused by files being held open by another process (e.g., AI coding tools, other cloud clients). (#16)
